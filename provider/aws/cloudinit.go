...
// BuildStartupScript renders the same bootstrap script without EC2 user-data
// compression/base64 wrapping. Providers such as GCP pass this plain script
// through instance metadata as startup-script.
func BuildStartupScript(p CloudInitParams) (string, error) {
	if err := validateParams(p); err != nil {
		return "", fmt.Errorf("invalid cloud-init parameters: %w", err)
	}
	return renderBootstrapScript(p)
}
