...
// GCP provisioning follows the same VPN/bootstrap lifecycle as AWS, but creates
// a Compute Engine VM and passes the bootstrap script through metadata.
func (r *NodeProvisionReconciler) reconcileGCPProvisioning(
	...
	if np.Status.InstanceID != "" {
		log.Info("GCP instance already exists, skipping creation", "instanceId", np.Status.InstanceID)
		if np.Status.Phase != mlv1alpha1.NodeProvisionPhaseWaitingForInstance {
			now := metav1.Now()
			np.Status.Phase = mlv1alpha1.NodeProvisionPhaseWaitingForInstance
			np.Status.LastUpdated = &now
	...
	patched, err := r.resolveGCPDefaults(ctx, np)
	if err != nil {
	...
	if err != nil {
		return r.failNodeProvision(ctx, np, fmt.Sprintf("GCP provisioning failed: %v", err))
	}
	log.Info("GCP instance creation requested", "instanceId", result.InstanceID)
...
func (r *NodeProvisionReconciler) resolveAWSDefaults(
	...
	if err := awsprovision.ValidateInstanceTypeAvailability(ctx, np.Spec.Region, np.Spec.AWSConfig.AvailabilityZone, np.Spec.InstanceType, creds); err != nil {
		return false, err
	}
	...
	if needsNetwork {
		...
		netCfg, err := awsprovision.ResolveOrCreateNetworkConfig(ctx, np.Spec.Region, np.Spec.AWSConfig.AvailabilityZone, creds)
		if err != nil {
		...
		log.Info("Resolved network config",
			"availabilityZone", np.Spec.AWSConfig.AvailabilityZone,
...
// resolveGCPDefaults auto-populates GCP fields that can be safely inferred.
func (r *NodeProvisionReconciler) resolveGCPDefaults(
	...
	needsConfig := np.Spec.GCPConfig == nil
	needsInstanceType := np.Spec.InstanceType == ""
	needsZoneMirror := np.Spec.GCPConfig != nil && np.Spec.GCPConfig.Zone == "" && np.Spec.Region != ""

	if !needsConfig && !needsInstanceType && !needsZoneMirror {
		return false, nil
	}
	...
	if needsInstanceType {
		it := gcpprovision.DefaultInstanceTypeForLabel(np.Spec.NodeLabel)
		if it == "" {
			return false, fmt.Errorf("spec.instanceType is required: no default GCP machine type defined for nodeLabel %q", np.Spec.NodeLabel)
		}
		np.Spec.InstanceType = it
		log.Info("Resolved GCP machine type from nodeLabel", "nodeLabel", np.Spec.NodeLabel, "instanceType", it)
	}
	if needsZoneMirror {
		np.Spec.GCPConfig.Zone = np.Spec.Region
		log.Info("Resolved GCP zone from spec.region", "zone", np.Spec.GCPConfig.Zone)
	}

	if err := r.Patch(ctx, np, client.MergeFrom(base)); err != nil {
		return false, fmt.Errorf("patching NodeProvision spec with resolved GCP defaults: %w", err)
	}
	log.Info("Patched NodeProvision spec with resolved GCP defaults")
	return true, nil
}

// reconcileWaitingForInstance polls the cloud instance until it is running,
// then transitions to the Joining phase.
func (r *NodeProvisionReconciler) reconcileWaitingForInstance(
	...
	case mlv1alpha1.CloudProviderAWS:
		...
		if err != nil {
			return ctrl.Result{}, fmt.Errorf("polling AWS instance state: %w", err)
		}
	case mlv1alpha1.CloudProviderGCP:
		var err error
		privateIP, publicIP, err = gcpprovision.WaitForInstanceRunning(ctx, np, gcpprovision.ResolveGCPCredentials(secret), np.Status.InstanceID)
		if err != nil {
			return ctrl.Result{}, fmt.Errorf("polling GCP instance state: %w", err)
		}
	default:
		return ctrl.Result{}, fmt.Errorf("waiting for instance is unsupported for provider %s", np.Spec.Provider)
	}
	if privateIP == "" {
...
func (r *NodeProvisionReconciler) handleDelete(ctx context.Context, np *mlv1alpha1.NodeProvision) (ctrl.Result, error) { //nolint:unparam
	...
	case mlv1alpha1.CloudProviderGCP:
		if np.Status.InstanceID != "" {
			...
			if err != nil {
				if !apierrors.IsNotFound(err) {
					log.Error(err, "getting credentials for GCP instance deletion, trying controller copy")
				} else {
					log.Info("User credentials secret not found, falling back to controller copy",
						"userSecret", np.Spec.CredentialsRef.Name)
				}
				secret, err = r.getControllerCredsSecret(ctx, np)
				if err != nil {
					return ctrl.Result{}, fmt.Errorf("no credentials to delete GCP instance %s; both user and controller-copy secrets unavailable: %w",
						np.Status.InstanceID, err)
				}
			}
			if err := gcpprovision.TerminateInstance(ctx, np, gcpprovision.ResolveGCPCredentials(secret), np.Status.InstanceID); err != nil {
				return ctrl.Result{}, fmt.Errorf("deleting GCP instance %s: %w", np.Status.InstanceID, err)
			}
